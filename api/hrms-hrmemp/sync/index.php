...
foreach ($changes as $index => $change) {
    ...
    if (!$okEmpCode) {
        log_message('invalid_emp_code', ['index' => $index, 'change_id' => $changeId]);
        respond(400, ['error' => 'Invalid EMP_CODE.', 'index' => $index]);
    }

    [$okCampLoc, $empCampLoc] = normalize_optional_string($change['EMP_CAMP_LOC'] ?? null, 10);
    if (!$okCampLoc) {
        log_message('invalid_emp_camp_loc', ['index' => $index, 'change_id' => $changeId]);
        respond(400, ['error' => 'Invalid EMP_CAMP_LOC.', 'index' => $index]);
    }
    ...
    $normalizedChanges[] = [
        'change_id' => $changeId,
        'emp_compcd' => $empCompCd,
        'emp_code' => $empCode,
        'emp_camp_loc' => $empCampLoc,
...
try {
    ...
    $stmtInboxExists = prepare_statement($bd, 'SELECT change_id FROM hrms_hrmemp_inbox WHERE change_id = ?', 'hrmemp_inbox_exists');
    ...
    $stmtUpsert = prepare_statement(
        ...
        'emp_camp_loc = VALUES(emp_camp_loc), ' .
    ...
    foreach ($normalizedChanges as $change) {
        ...
        try {
            ...
            $empCompCd = $change['emp_compcd'];
            $empCode = $change['emp_code'];
            $empCampLoc = $change['emp_camp_loc'];
            ...
            if (!$stmtUpsert->bind_param(
                ...
                throw new RuntimeException('HRMEMP upsert bind failed: ' . $stmtUpsert->error);
            }
            if (!$stmtUpsert->execute()) {
                throw new RuntimeException('HRMEMP upsert execute failed: ' . $stmtUpsert->error);
            }
...
function resolve_api_key(mysqli $bd): string
{
    ...
    $configKey = trim((string) ($config['api_key_config_key'] ?? 'hrms_hrmemp_sync_api_key'));
    if ($configKey !== '') {
    ...
    $envKey = getenv('HRMS_HRMEMP_SYNC_API_KEY');
...
function resolve_log_path(): string
{
    ...
    $envPath = getenv('HRMS_HRMEMP_SYNC_LOG_PATH');
    if (is_string($envPath) && $envPath !== '') {
        return $envPath;
    }

    return dirname(__DIR__) . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'hrms_hrmemp_sync.log';
}
...
function open_db(): mysqli
{
    ...
    if ($dbName === '') {
        $envName = getenv('HRMS_HRMEMP_SYNC_DB_NAME');
...
function ensure_tables(mysqli $bd): void
{
    if (!$bd->query(
        ...
        'PRIMARY KEY (emp_compcd, emp_code),' .
        'KEY idx_hrmemp_camp_loc (emp_camp_loc),' .
        'KEY idx_hrmemp_changed_at (changed_at),' .
    ...
    if (!$bd->query(
        'CREATE TABLE IF NOT EXISTS hrms_hrmemp_inbox (' .
        ...
        throw new RuntimeException('Failed to create hrms_hrmemp_inbox table: ' . $bd->error);
    }
}
