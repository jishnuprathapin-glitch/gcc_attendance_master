...
foreach ($changes as $index => $change) {
    ...
    $punchDate = normalize_date($change['punchDate'] ?? null);
    if ($punchDate === null) {
        log_message('invalid_punch_date', [
            'index' => $index,
            'change_id' => $changeId,
            'punch_date' => (string) ($change['punchDate'] ?? ''),
        ]);
        respond(400, ['error' => 'Invalid punchDate.', 'index' => $index]);
    }

    $firstLog = normalize_datetime($change['firstLog'] ?? null);
    $lastLog = normalize_datetime($change['lastLog'] ?? null);
    $firstTerminalSn = normalize_optional_string($change['firstTerminalSn'] ?? null);
    $lastTerminalSn = normalize_optional_string($change['lastTerminalSn'] ?? null);
    ...
    $normalizedChanges[] = [
        ...
        'first_log' => $firstLog,
        'last_log' => $lastLog,
        'first_terminal_sn' => $firstTerminalSn,
        'last_terminal_sn' => $lastTerminalSn,
...
try {
    ...
    $stmtChangeInsert = prepare_statement(
        $bd,
        'INSERT IGNORE INTO employee_daily_punch ' .
        '(change_id, emp_code, punch_date, first_log, last_log, first_terminal_sn, last_terminal_sn, change_type, changed_at) ' .
        'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        'changes_insert'
    );
    ...
    foreach ($normalizedChanges as $change) {
        ...
        $punchDate = $change['punch_date'];
        $firstLog = $change['first_log'];
        $lastLog = $change['last_log'];
        $firstTerminalSn = $change['first_terminal_sn'];
        $lastTerminalSn = $change['last_terminal_sn'];
...
function resolve_api_key(): string
{
    ...
    $envKey = getenv('EMPLOYEE_DAILY_PUNCH_API_KEY');
...
function resolve_log_path(): string
{
    ...
    $envPath = getenv('EMPLOYEE_DAILY_PUNCH_LOG_PATH');
    if (is_string($envPath) && $envPath !== '') {
        return $envPath;
    }

    return dirname(__DIR__) . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'employee_daily_punch_sync.log';
}
...
function open_db(): mysqli
{
    ...
    if ($dbName === '') {
        $envName = getenv('EMPLOYEE_DAILY_PUNCH_DB_NAME');
...
function ensure_tables(mysqli $bd): void
{
    if (!$bd->query(
        ...
        log_message('table_create_failed', [
            'table' => 'employee_daily_punch',
            'error' => $bd->error,
        ]);
        throw new RuntimeException('Failed to create employee_daily_punch table.');
    }

    ensure_table_columns($bd, 'employee_daily_punch', [
        'first_log' => 'datetime NULL',
        'last_log' => 'datetime NULL',
        'first_terminal_sn' => 'varchar(50) NULL',
        'last_terminal_sn' => 'varchar(50) NULL',
